...
| `board show [-x] [--attention] [--json]` | the same, one-shot; every card names its agent, its role when it is not a plain implementer (`sub-orchestrator`, `spec reviewer`), and `by <lead>` when a lead dispatched it; subtasks (`1.1`) carry a `↳` under their parent — cheap for the controller to glance at; `--json` for tooling |
