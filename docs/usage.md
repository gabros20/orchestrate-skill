...
board learn 4 "jest fixture db.sqlite is stale — run npm run db:reset first" --agent impl-4 --kind gotcha --path tests/   # one line a peer would re-pay; reaches its team at their next board call
...
board wait --agent lead-1 --task 1.1 --task 1.2 --timeout 0 --or-mail   # a lead waits on its workers; exit 5 = one of them asked the lead something first
...
thing per state: `~ after N` queued, `> elapsed`, `! stale`, `? asked controller #n`, `x reason`, gate
...
| `board check [--finish] [--replay] [--json]` | reconciliation, exit 1 on findings: journal chain breaks, dispatched-never-returned, stale in-progress, silent reviewers, report without review, ledger lines naming commits or artifacts that don't exist, done over a failed gate or check, done with no review gate ever opened, quality gate before spec, xcli DONE without commits or without a usage receipt, a third attempt with no escalation, tasks skipped while later ones are done, model drift, budget overrun, unapproved launch, `finish` with open tasks or invalidated by later work / a moved HEAD / a changed goal. `--finish` is the strict pre-finish gate; a brief or card on disk for a task the plan never queued; `--replay` re-derives every card from the journal alone and reports any fact that reached the disk without its journal line (`inconclusive` where the pointers are gone). A note (never a finding) when the `board` on PATH differs from the running script: engines run commands through a login shell, so a stale installed copy wins over the wrapper's PATH — `install.sh` refreshes it. |
| `board runs` | every run this machine knows about — repo, run id, last activity, workspace path — newest first |
| `board version [--json] [--ack V --msg …]` · `board --version` | the board's release and journal schema. On a run: the release it started on (every event carries `v`), each release since with its migration line — `none` (additive, upgrade in place) or `action` (a `check` finding, so finish refuses, until `--ack`) — and mixed writers (a stale copy on some lane's PATH). An older board never writes a newer-schema journal |
| `board reap [--agent A] [--kill --all --pid P --keep P --why …]` | what the run left running — lane trees, and orphans whose cwd is in the run's trees, owned by the stint whose window holds their start. Lists by default; `--kill` stops TERM→KILL (pid re-verified; an open lane is stopped engine-first, then `board return` it); `--all` adds owner-unknown and live lanes (the run's kill switch); `--keep PID --why` records an intended survivor. `lane-exit` journals leftovers, `check` flags them, the finish gate refuses them. Inside a lane: own processes only |
| `board launch N --agent A --engine E --model M [--effort --owns --after 1,2 --extra=… --dry-run --fg] -- spec.md` | one command per lane (`--after` gates the lane on other tasks' work being on disk — the **wrapper** waits, the model never starts until then, so waiting costs zero tokens; the card shows `~ queued after task 1`): the engine's argv from one table (approval/sandbox flags only via `--extra`, knowingly), session pinned (Grok, Claude) or captured from the log (Codex), wrapper under `raw/`, dispatch journaled with pid, detached start; on exit an `exit` event (a dead lane shows in attention before the stale clock) and a `receipt` event (tokens · calls · cost, per engine). `--dry-run` prints the wrapper and journals nothing. A lane whose dependency cannot land exits **125** without ever starting its engine (`x` in attention: return it BLOCKED, fix the dependency, relaunch); the wrapper exports `ORCHESTRATE_WS` and puts `board` on PATH, so a lane in any worktree journals to the right run. |
...
| `board mail [--task N]` · `board inbox --agent A` · `board peers --agent A` | the conversation threaded by reply, an agent's unread (tagged `[controller]` or `[lead x]` = instruction, `[peer x]` = information; reading journals an `ack`), and who else is open and what they own — its own team first, other subtrees after. `controller` and `lead` in `--to` are relative: a worker dispatched `--by` a lead reaches that lead, and the controller sees the ask only once the lead leaves it unread past the stale threshold. Loop-proof by construction: 8 sends per stint, threads 3 deep, no duplicates, no mail to a returned agent — beyond that the answer belongs in the report. Delivery rides on the recipient's own board calls (`note`, `exec`, `send`, `peers`, `vote` all hand over unread mail) plus an inbox read at start and before the report; `--push` prints the host's native binding (never runs it). In a read-only sandbox lane (Codex `--sandbox read-only`, Claude `--restricted`) every worker-side command prints the line to carry in the inline return instead of failing; `board exec` keeps one log per run (`raw/check-taskN-<name>-<seq>.log`), so a re-run never erases evidence. |
| `board learn [N "…" --agent A --kind K --path P]` | with arguments: a worker's one-line learning — capped at 3 per stint and 300 chars, deduped run-wide, journaled, never a transition; team-scoped delivery (same dispatcher, the sender's lead, its own workers; the controller sees all). Without arguments: the run's learnings. Every worker-side board call hands over one **digest** under one watermark: unread mail, learnings since its last check, and the decisions and rails recorded after its dispatch (its brief snapshot predates them — reconcile, or return NEEDS_CONTEXT); the newest 12 learnings are shown, the count of earlier ones named; `--peek` acks nothing. Inside a launched lane `BOARD_AGENT` is set, so no worker-side command needs `--agent`. |
| `board memory [--observation\|--handoff] [--scope a,b] [--out PATH]` | the run rendered as one `project-context` record: decisions with their why (or the honest "not recorded"), attempts (`learn --kind failed`, BLOCKED/REFUSED returns of stints that actually ran — a lane that died before its engine did anything is not an attempt), learnings, commits of accepted stints, artifacts (paths only), checks as verification, the frontier as `current_state`; an observation when the final gate stands, else a handoff. Printed or written, never appended: the board prints the `ctx append` / `ctx handoff` line at `init`, `finish` and `resume` only when the repo keeps `.agent/PROJECT_CONTEXT.jsonl`, and never runs `ctx`. |
...
A run's workspace is archived by the next kickoff; what it learned survives only when the repo
keeps project memory — the [`project-context`](https://github.com/gabros20/project-context) skill's
`.agent/PROJECT_CONTEXT.jsonl`, an append-only ledger any coding agent reads and writes. Orchestrate
never depends on it: when `ctx init` has run in the repo, `board init` names the packet command the
controller pins in briefs (`ctx context --scope <x> --budget 1500 > .orchestrate/memory-<x>.md`),
and `board finish` / `board resume` name the record command (`board memory --out … && ctx append
--agent orchestrate --session-id <run> --input …`). Workers never run `ctx` — a linked worktree
has no ledger — they receive memory as brief pointers and add to it through `board learn`. No
`.agent/` → nothing printed, nothing changes.
