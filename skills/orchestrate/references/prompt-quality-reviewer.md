...
Purpose: Provide the review prompt for correctness, maintainability, and implementation quality after spec compliance.

Read when:
- A task passed spec review and quality review is enabled.

Skip when:
- The task has not passed spec review or review is disabled.

Inputs:
- Task brief, diff, spec findings, repository conventions, and findings path.
