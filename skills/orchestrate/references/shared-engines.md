...
|---|---|---|---|---|---|---|
| Codex | `thread.started` in `--json` | `codex exec resume <id> - < nudge.md` | `turn.completed.usage` (root turn only) | quota + `--worktree` | spec-preamble opt-out of `~/.codex/AGENTS.md` | dry-run |
| Grok | `-s $(uuidgen)` pre-pinned | `grok -r <id> --prompt-file` | `grok usage <id>` | `--max-turns` | `--no-subagents` | **live** 2026-09-16 |
| Claude | `--session-id $(uuidgen)` | `claude -p --resume <id>` | final `result` event | `--max-budget-usd` | `--bare`, `--restricted` | dry-run |
