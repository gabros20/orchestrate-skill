...
Purpose: Provide role-scoped communication blocks and briefing rules that remove waste without underpriming agents.

Read when:
- Any worker, reviewer, advisor, verifier, or controller handoff is being written.

Skip when:
- No agent-to-agent instruction or report is involved.

Inputs:
- Role, task contract, context pointers, report schema, and output limits.

Produces:
- Complete low-noise dispatch text and dense structured returns.
...
6. **Report contract** — schema, cap, file path to write it. Optionally one heartbeat line:
   `board note N "<msg>"` on milestones (tests green, committing) — a note, never a status; the
   controller alone moves cards (`shared-contracts.md`).
